...
func _confirm_target(target):
	...
	# --- Play attack animation if it's a physical move ---
	var physical_moves = ["attack", "double_strike", "lifesteal"]
	if current_move_type in physical_moves:
		sprite.play("attack")
...
func _cancel_targeting(targets):
	...
	if current_move_type == "attack":
		...
		# Returns to magic menu for fireball/heal/skills
		skill_menu.show()
...
func _on_double_strike_button_pressed() -> void:
	# Triggers enemy targeting for Double Strike
	_start_targeting("double_strike", false)

func _on_lifesteal_button_pressed() -> void:
	# Triggers enemy targeting for Lifesteal
	_start_targeting("lifesteal", false)
...
func _on_magic_pressed() -> void:
	...
	var ds_btn = skill_menu.find_child("double_strike") # Ensure this matches Node name
	var ls_btn = skill_menu.find_child("lifesteal")    # Ensure this matches Node name
	
	if fireball_btn:
		fireball_btn.disabled = active_player.mana < move_costs["fireball"]
	
	if ds_btn:
		ds_btn.disabled = active_player.mana < move_costs["double_strike"]
		
	if ls_btn:
		ls_btn.disabled = active_player.mana < move_costs["lifesteal"]
	...
	elif ds_btn and not ds_btn.disabled:
		ds_btn.grab_focus()
	elif ls_btn and not ls_btn.disabled:
		ls_btn.grab_focus()
